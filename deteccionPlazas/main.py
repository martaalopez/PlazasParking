...
# Cargar las posiciones guardadas de las plazas de estacionamiento
...
def check(img, imgPro):
    ...
    for pos in posList:
        # Asegurarse de que pos es una tupla con dos tuplas (coordenadas de inicio y fin)
        if isinstance(pos, tuple) and len(pos) == 2:
            ...
            if isinstance(start, tuple) and isinstance(end, tuple) and len(start) == 2 and len(end) == 2:
                x1, y1 = start  # Coordenadas de inicio (x1, y1)
                x2, y2 = end    # Coordenadas de fin (x2, y2)
                ...
                if x1 < 0 or y1 < 0 or x2 > imgWidth or y2 > imgHeight:
                    ...
                    continue  # Ignorar esta posición si está fuera de los límites
...
img = cv2.imread("parking1.png")  # Cambia el nombre por el archivo de tu imagen
